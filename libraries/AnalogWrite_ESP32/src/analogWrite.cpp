...
void analogWriteFrequenci(uint32_t frequency) {
  for (uint8_t i = 0; i < 16; i++) {
...
void analogWriteFrequenci(int8_t pin, uint32_t frequency) {
...
void analogWriteResolutionn(uint8_t resolution) {
...
void analogWriteResolutionn(int8_t pin, uint8_t resolution) {
  uint8_t channel = analogWriteChannel(pin);
  // Make sure the pin was attached to a channel, if not do nothing
  if (channel < 16) {
    if (resolution < 1) resolution = 1;
